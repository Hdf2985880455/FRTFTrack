...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    ...
    if use_ci2p:
        # CI2P reduces the patch token grid by `compression_ratio`,
        # but the backbone patch stride is still STRIDE=16. Relative to the
        # baseline token grid (template_size / stride), the effective divisor is:
        #   stride * (compression_ratio / 2)
        # so:
        #   ratio=2 -> template 192 => 12x12 tokens
        #   ratio=4 -> template 192 => 6x6 tokens
        stride_for_feat_size = int(stride * (compression_ratio / 2))
    else:
        stride_for_feat_size = stride
    template_feat_size = template_size // stride_for_feat_size

    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'ALL':
        box_mask_z = None
    elif cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'CTR_POINT':
        if template_feat_size == 8:
            index = slice(3, 4)  # center point index
        elif template_feat_size == 12:
            index = slice(5, 6)
        elif template_feat_size == 6:
            # for even grid, the codebook uses index = (N/2 - 1)
            # e.g., 8 -> 3, 12 -> 5, 14 -> 6
            index = slice(2, 3)
        ...
        box_mask_z = torch.zeros([bs, template_feat_size, template_feat_size], device=device) # (B, 8, 8)
        box_mask_z[:, index, index] = 1   # center point = 1
        box_mask_z = box_mask_z.flatten(1).to(torch.bool)  # (B, 8, 8) to (B, 64)
        ...
        if template_feat_size == 8:
            ...
            # keep a 2x2 region around the center for 6x6 (2..3)
            index = slice(2, 4)
        elif template_feat_size == 7:
            index = slice(3, 4)
        else:
        ...
        box_mask_z = F.interpolate(box_mask_z, scale_factor=1. / stride_for_feat_size, mode='bilinear',
                                   align_corners=False)
        box_mask_z = box_mask_z.flatten(1).to(torch.bool)
