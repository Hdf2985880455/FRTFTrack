...
from lib.models.mmtrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    @staticmethod
    def _region_token_grid_hw(H, W, patch_stride, patch_embed):
        """Token grid (H_tok, W_tok) for an H×W image; must match patch_embed(z/x) length."""
        if hasattr(patch_embed, "get_output_hw"):
             return patch_embed.get_output_hw(H, W)
        return H // patch_stride, W // patch_stride
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        # 对于标准 ViT，Q 来自原始 patch grid (例如 14x14=196)；
        # 对于 CI2P，Q 会变成新的 grid (例如 7x7=49)，此时应使用实际 patch_embed 的 grid_size。
        if hasattr(self.patch_embed, "grid_size"):
            P_H, P_W = self.patch_embed.grid_size
        else:
            # 回退：从 Q 推出一个近似的正方形网格
            P_H = int(Q ** 0.5)
            P_W = Q // P_H
        assert P_H * P_W == Q, f"Positional embedding length {Q} cannot be reshaped to {P_H}x{P_W}"
        patch_pos_embed = patch_pos_embed.view(B, E, P_H, P_W)

        # for search region
        H, W = search_size
        new_P_H, new_P_W = self._region_token_grid_hw(H, W, new_patch_size, self.patch_embed)
