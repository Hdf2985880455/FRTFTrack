...
def get_optimizer_scheduler(net, cfg, settings):
    ...
    ci2p_lr_mult = float(getattr(cfg.TRAIN, "CI2P_LR_MULT", 1.0))

    normal_params = []
    ci2p_params = []

    if use_ci2p:
        print("Training Vision-Language Tracking Task with CI2P: Freeze visual encoder except CI2P patch embedding...")

        ci2p_trainable_keys = (
            #     "backbone.pos_embed",
            #     "backbone.pos_embed_z",
            #     "backbone.pos_embed_x",
                "backbone.patch_embed.compression_encoder",
        ...
        for n, p in net.named_parameters():
            if "backbone" in n and p.requires_grad:
                ...
                # 再放开 CI2P 相关层
                if any(k in n for k in ci2p_trainable_keys):
                    p.requires_grad = True
                    ci2p_params.append(p)
                    print("Unfreeze (CI2P):", n)
                else:
                    print("Freeze backbone:", n)
            else:
                if p.requires_grad:
                    normal_params.append(p)
    else:
        print("Training baseline: Freeze visual encoder (original behavior).")
    ...
    if is_main_process():
        ...
        print(f"Total learnable params: {total_learnable / 1e6:.3f} M")
        print(f"CI2P learnable params: {total_ci2p / 1e6:.3f} M")
        print("=" * 80)

    if len(ci2p_params) > 0 and ci2p_lr_mult != 1.0:
        param_dicts = [
            {"params": normal_params, "lr": cfg.TRAIN.LR},
            {"params": ci2p_params, "lr": cfg.TRAIN.LR * ci2p_lr_mult},
        ]
    else:
        param_dicts = [
            {"params": normal_params + ci2p_params, "lr": cfg.TRAIN.LR}
        ]
