...
class MMTrackActor(BaseActor):
    ...
    def _batch_temporal_valid(self, data):
        temporal_valid = data.get('temporal_valid', False)
        if isinstance(temporal_valid, torch.Tensor):
            if temporal_valid.numel() == 0:
                return False
            return bool(temporal_valid.bool().all().item())
        if isinstance(temporal_valid, (list, tuple)):
            if len(temporal_valid) == 0:
                return False
            return all(bool(x) for x in temporal_valid)
        return bool(temporal_valid)

    def _normalize_exp_str(self, exp_str):
        ...
        if isinstance(exp_str, torch.Tensor):
            raise TypeError("exp_str should not be a tensor")

        if isinstance(exp_str, (list, tuple)):
            # Keep one language string per sample. Do not flatten across the
            # batch dimension, otherwise the text batch can silently become
            # shorter than the visual batch (e.g. when some entries are None).
            return [_extract_single(item) for item in exp_str]
    ...
    def __call__(self, data):
        ...
        temporal_valid = self._batch_temporal_valid(data)
        target_search_anno = data['search_anno'][1].unsqueeze(0) if (
            temporal_valid and len(data['search_images']) >= 2
        ) else data['search_anno'][0].unsqueeze(0)
        ...
        loss, status = self.compute_losses(
            ...
            bbox_anno=target_search_anno if with_bbox else None,
            with_bbox=with_bbox,
            with_mask=with_mask
        )
        ...
        if with_bbox and with_mask:
            bbox_acc, mask_iou, mask_acc = self.calc_accuracy(out_dict, bbox_anno=target_search_anno, mask_anno=data["search_masks"])
        elif with_bbox:
            bbox_acc, mask_iou, mask_acc = self.calc_accuracy(out_dict, bbox_anno=target_search_anno)
        elif with_mask:
        ...
        if with_bbox:
            ...
            # Diagnose potential bbox collapse (width/height/area near zero).
            # pred_bboxes is expected to be normalized xyxy in [0,1] range.
            with torch.no_grad():
                pb = out_dict.get('pred_bboxes', None)
                if isinstance(pb, torch.Tensor) and pb.numel() >= 4:
                    x1, y1, x2, y2 = pb[:, 0], pb[:, 1], pb[:, 2], pb[:, 3]
                    w = (x2 - x1).clamp(min=0.0)
                    h = (y2 - y1).clamp(min=0.0)
                    area = w * h
                    status['BBox/w_mean'] = w.mean().item()
                    status['BBox/h_mean'] = h.mean().item()
                    status['BBox/area_mean'] = area.mean().item()
                    status['BBox/tiny_frac(<1e-4)'] = (area < 1e-4).float().mean().item()
    ...
    def forward_pass(self, data, with_bbox=False, with_mask=False, with_language=False):
        ...
        exp_str = self._normalize_exp_str(data['exp_str'])
        ...
        if (
            ...
            and with_language
            and temporal_valid
            and len(data['search_images']) >= 2
        ):
            search_prev = data['search_images'][0].view(-1, *data['search_images'].shape[2:])
            search_curr = data['search_images'][1].view(-1, *data['search_images'].shape[2:])

            net = self.net.module if multigpu.is_multi_gpu(self.net) else self.net
            out_dict = net.forward_temporal(
                template=template_list,
                search_prev=search_prev,
                search_curr=search_curr,
                search_prev_attn_mask=data['search_att'][0],
                search_curr_attn_mask=data['search_att'][1],
                search_prev_anno=data['search_anno'][0].unsqueeze(0),
                search_curr_anno=data['search_anno'][1].unsqueeze(0),
                exp_str=exp_str,
        ...
        # fallback single-search path for baseline-compatible samples
        assert len(data['search_images']) >= 1
        search_img = data['search_images'][0].view(-1, *data['search_images'].shape[2:])  # (batch, 3, 320, 320)
        search_anno = data['search_anno'][0].unsqueeze(0)
        ...
        if with_bbox and with_mask:
            ...
            out_dict = self.net(template=template_list,
                                ...
                                search_anno=search_anno,
                                search_attn_mask=data['search_att'][0],
                                exp_str=exp_str,               # language descriptions
                                ce_template_mask=box_mask_z,
                                ce_keep_rate=ce_keep_rate,
                                return_last_attn=False)
        elif with_bbox:
    ...
    def build_reliability_targets(self, pred_bboxes, bbox_anno):
        """
        Args:
            pred_bboxes: [B, 4] normalized xyxy
            bbox_anno: [1, B, 4] normalized xywh
        Returns:
            rel_targets: [B]
            rel_weights: [B]
        """
        pos_thr = getattr(self.cfg.TRAIN, "REL_POS_THR", 0.5)
        neg_thr = getattr(self.cfg.TRAIN, "REL_NEG_THR", 0.3)
        ignore_mid = getattr(self.cfg.TRAIN, "REL_IGNORE_MID", True)
        soft_target = getattr(self.cfg.TRAIN, "REL_SOFT_TARGET", False)

        gt_xyxy = box_xywh_to_xyxy(bbox_anno)[0]
        ious, _ = box_iou(pred_bboxes, gt_xyxy)

        if soft_target:
            denom = max(pos_thr - neg_thr, 1e-6)
            rel_targets = ((ious - neg_thr) / denom).clamp(0.0, 1.0)
            rel_weights = pred_bboxes.new_ones((pred_bboxes.size(0),))
            return rel_targets, rel_weights

        rel_targets = pred_bboxes.new_full((pred_bboxes.size(0),), -1.0)
        rel_targets[ious >= pos_thr] = 1.0
        rel_targets[ious <= neg_thr] = 0.0

        if ignore_mid:
            rel_weights = (rel_targets >= 0).float()
        else:
            rel_weights = pred_bboxes.new_ones((pred_bboxes.size(0),))
            mid_mask = rel_targets < 0
            rel_targets[mid_mask] = 0.5

        return rel_targets, rel_weights

    def compute_losses(self, out_dict, targets, aux_outputs=None, bbox_anno=None,
                       with_bbox=False, with_mask=False, return_status=True):
        """Args:
        ...
        pred_reliability = out_dict.get("pred_reliability", None)

        if with_bbox and pred_reliability is not None and bbox_anno is not None:
            with torch.no_grad():
                rel_targets, rel_weights = self.build_reliability_targets(
                    pred_bboxes=out_dict["pred_bboxes"].detach(),
                    bbox_anno=bbox_anno
                )

            valid_mask = rel_weights > 0
            if valid_mask.any():
                rel_logits = pred_reliability.view(-1)
                rel_loss_raw = F.binary_cross_entropy_with_logits(
                    rel_logits[valid_mask],
                    rel_targets[valid_mask],
                    reduction="mean"
                )
                rel_loss = rel_loss_raw * getattr(self.cfg.TRAIN, "REL_LOSS_WEIGHT", 0.5)
                loss.append(rel_loss)

        if return_status:
            # status for log
            loss_status["Loss/cls"] = ce_loss.item()
            loss_status["Loss/rel"] = rel_loss.item()
            loss_status["Loss/total"] = sum(loss).item()

            if pred_reliability is not None:
                with torch.no_grad():
                    rel_prob = torch.sigmoid(pred_reliability.view(-1))
                    loss_status["Rel/mean"] = rel_prob.mean().item()

                    if with_bbox and bbox_anno is not None:
                        rel_targets, rel_weights = self.build_reliability_targets(
                            pred_bboxes=out_dict["pred_bboxes"].detach(),
                            bbox_anno=bbox_anno
                        )
                        valid_mask = rel_weights > 0
                        if valid_mask.any():
                            loss_status["Rel/target_mean"] = rel_targets[valid_mask].mean().item()
                        else:
                            loss_status["Rel/target_mean"] = 0.0
                    else:
                        loss_status["Rel/target_mean"] = 0.0
